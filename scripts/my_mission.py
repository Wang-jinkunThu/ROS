...
class StateMachine:
    ...
    def do_detect_boll(self):
        ...
        self.uav.position_change(0.5, -1.25, 1.25, 90)
        self.uav.position_control_demo()
        self.uav.position_change(0.5, -1.25, 1.25, 0)
        self.uav.position_control_demo()
    ...
    def do_detect_light(self):
        ...
        # 观察点（米）
        obs_x, obs_y, obs_z = 0.1, 0.5, 1.63

        # 移动到观察点，面朝白板（yaw=90）
        rospy.loginfo(f"Going to observation point ({obs_x}, {obs_y}, {obs_z})...")
        ...
        for light_num in range(1, 4):
            ...
            # 飞向目标位置（cm → m）
            self.uav.position_change(target_x / 100, target_y / 100, target_z / 100, 90)
            self.uav.position_control_demo()

            # 发送裁判机目标
            self.uav.judge_pub.publish("target")
            print(f"[JUDGE] Target {light_num} sent")
            rospy.sleep(1)
